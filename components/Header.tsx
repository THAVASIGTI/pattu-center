...
function Brand({ dark = false }: { dark?: boolean }) {
  return (
    <Link href="/" className="flex min-w-0 items-center gap-3" aria-label={`${business.name} — home`}>
      <span className="grad-royal grid size-[42px] shrink-0 place-items-center rounded-full border-[1.5px] border-gold shadow-[0_3px_14px_rgba(10,23,56,.34)]">
        <span className="font-serif text-[1.05rem] tracking-wide text-gold-light">{business.initials}</span>
      </span>
      <span className={`min-w-0 font-serif text-[1.02rem] leading-tight lg:whitespace-nowrap lg:text-[1.08rem] ${dark ? "text-white" : "text-royal-deep"}`}>
        {business.name}
...
export default function Header() {
  ...
  return (
    <>
      {/* Top bar */}
      <div className="border-b border-gold/25 bg-[linear-gradient(90deg,#0a1738_0%,#1e40af_50%,#0a1738_100%)] text-[0.79rem] text-gold-pale lg:text-[0.82rem]">
        <Wrap className="flex min-h-[34px] flex-wrap items-center justify-center gap-4 lg:min-h-10 lg:justify-between">
          <span className="font-tamil text-gold-light">{business.tagline}</span>
      ...
      <header
        className={`sticky top-0 z-50 border-b border-line bg-cream/95 backdrop-blur-md transition-shadow duration-300 ${
          stuck ? "shadow-[0_6px_24px_rgba(10,23,56,.11)]" : ""
        }`}
      >
        <Wrap className="flex min-h-[62px] items-center justify-between gap-3.5 lg:min-h-[72px]">
              ...
              <Link
                ...
                className={`whitespace-nowrap rounded-[9px] px-2.5 py-2 text-[0.85rem] transition-colors ${
                  isActive(l.href)
                    ...
                    : "font-medium text-ink-soft hover:bg-royal/[0.07] hover:text-royal"
                }`}
              >
                {l.label}
              </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="grid size-[46px] shrink-0 cursor-pointer place-items-center rounded-xl border border-line-gold bg-white text-royal xl:hidden"
            >
      ...
      <div
        ...
        className={`fixed inset-0 z-[88] bg-[rgba(6,14,36,.62)] backdrop-blur-[3px] transition-opacity duration-300 xl:hidden ${
      ...
      <nav
        id="drawer-nav"
        aria-label="Mobile"
        className={`fixed top-0 right-0 z-[90] flex h-dvh w-[min(86vw,340px)] flex-col overflow-y-auto overscroll-contain border-l border-gold/30 px-5 pt-[18px] pb-[calc(22px+env(safe-area-inset-bottom))] transition-transform duration-[360ms] ease-out xl:hidden bg-[radial-gradient(420px_360px_at_100%_0%,rgba(58,99,216,.4),transparent_68%),linear-gradient(168deg,#1a3480_0%,#0a1738_100%)] ${
          open ? "translate-x-0" : "translate-x-[102%]"
        }`}
      >
        <div className="mb-4 flex items-center justify-between">
          <span className="text-[0.7rem] font-semibold uppercase tracking-[0.2em] text-gold-light">Menu</span>
          <button
            type="button"
            onClick={() => setOpen(false)}
            aria-label="Close menu"
            className="grid size-[42px] cursor-pointer place-items-center rounded-xl border border-gold/35 text-gold-light"
          >
        ...
        {navLinks.map((l) => (
          <Link
            ...
            href={l.href}
            aria-current={isActive(l.href) ? "page" : undefined}
            className={`flex min-h-[52px] items-center justify-between border-b border-gold/20 px-1 py-2.5 text-base last:border-b-0 ${
              isActive(l.href) ? "font-semibold text-gold-light" : "text-cream/85"
            }`}
          >
            {l.label}
            <span className="text-[1.3rem] leading-none text-gold">›</span>
          </Link>
        ))}
